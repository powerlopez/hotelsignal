...
import { hotel, findings, analysisCycles } from '@/data/hotel-data'
...
export default function DashboardPage() {
  ...
  const highPriority = recommendations.filter(r => r.priority >= 4 && r.status !== 'done' && r.status !== 'dismissed')

  // Economic impact calculation
  const highImpactRecs = recommendations.filter(
    r => r.priority >= 4 && r.status !== 'done' && r.status !== 'dismissed'
  )
  const totalMinBookings = highImpactRecs.reduce((s, r) => s + r.estimated_bookings_min, 0)
  const totalMaxBookings = highImpactRecs.reduce((s, r) => s + r.estimated_bookings_max, 0)
  const avgBookingValue = hotel.adr_eur * hotel.avg_stay_nights
  const commissionSavedMin = Math.round(totalMinBookings * avgBookingValue * (hotel.ota_commission_pct / 100))
  const commissionSavedMax = Math.round(totalMaxBookings * avgBookingValue * (hotel.ota_commission_pct / 100))

  // Top 3 high-priority findings for overview
  const topFindings = findings
    .filter(f => f.impact_level === 'high')
    .slice(0, 3)

  const latestCycle = analysisCycles[0]
    ...
    <div className="p-8 max-w-6xl mx-auto">
      ...
      <div className="mb-8">
        <p className="text-sm text-slate-500 mb-1">
          Análisis del {new Date(latestCycle.triggered_at).toLocaleDateString('es-ES', { day: 'numeric', month: 'long', year: 'numeric' })}
        </p>
        <h1 className="text-2xl font-bold text-slate-900">Resumen comercial</h1>
        <p className="text-slate-500 mt-1">
          {latestCycle.findings_count} hallazgos detectados · {pending + inProgress} acciones pendientes
        </p>
      </div>
      ...
      <div className="grid grid-cols-4 gap-4 mb-8">
        <StatCard
          ...
          value={latestCycle.findings_count}
      ...
      <div className="grid grid-cols-3 gap-6 mb-8">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-6 flex flex-col items-center justify-center gap-4">
          <div className="text-center">
            ...
            <OpportunityScore score={hotel.opportunity_score} size={160} />
          </div>
          <p className="text-xs text-slate-400 text-center">
            Implementa las 3 acciones prioritarias para subir a{' '}
            <span className="font-semibold text-brand-600">73/100</span>
          </p>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-6">
          ...
          <ScoreBreakdown breakdown={hotel.score_breakdown} />
        </div>
        ...
        <div className="bg-gradient-to-br from-amber-50 to-orange-50 rounded-2xl border border-amber-200 p-6">
          ...
          <p className="text-xs text-amber-700 mb-4">
            Si implementas las {highImpactRecs.length} recomendaciones prioritarias pendientes:
          </p>

          <div className="mb-4">
            <div className="text-2xl font-bold text-amber-900">
              +{totalMinBookings}–{totalMaxBookings}
            </div>
          ...
          <div className="bg-white/60 rounded-xl p-3 mb-3">
            <div className="text-lg font-bold text-amber-900">
              €{commissionSavedMin.toLocaleString('es-ES')} – €{commissionSavedMax.toLocaleString('es-ES')}/mes
            </div>
            <div className="text-xs text-amber-700">ahorro estimado en comisiones OTA</div>
          </div>

          <p className="text-xs text-amber-600/70">
            ADR €{hotel.adr_eur} · estancia media {hotel.avg_stay_nights}n · comisión OTA {hotel.ota_commission_pct}%
          </p>
          <p className="text-xs text-amber-600/60 mt-2 italic">
            * Estimación basada en buenas prácticas del sector. No garantiza resultados.
          </p>
        </div>
      </div>
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-1 gap-4">
          {topFindings.map(finding => {
            const rec = recommendations.find(r => r.finding_id === finding.id)
            return (
              <Link
                key={finding.id}
              ...
              >
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <h3 className="text-sm font-semibold text-slate-900 mb-1 group-hover:text-brand-700">
                      {finding.title}
                    </h3>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-sm text-slate-500">
            {done} de {recommendations.filter(r => r.status !== 'dismissed').length} completadas
          </span>
        </div>
        <div className="h-3 bg-slate-100 rounded-full overflow-hidden mb-4">
          <div
            ...
            style={{
              width: `${Math.round((done / recommendations.filter(r => r.status !== 'dismissed').length) * 100)}%`,
            }}
          />
        </div>
        <div className="flex gap-4 text-xs">
          <span className="flex items-center gap-1.5 text-slate-500">
            <span className="w-2 h-2 rounded-full bg-brand-500" /> Completadas: {done}
          </span>
          <span className="flex items-center gap-1.5 text-slate-500">
            <span className="w-2 h-2 rounded-full bg-blue-400" /> En progreso: {inProgress}
          </span>
          <span className="flex items-center gap-1.5 text-slate-500">
            <span className="w-2 h-2 rounded-full bg-slate-300" /> Pendientes: {pending}
          </span>
        </div>
      </div>
    </div>
  ...
  return (
    <div
      className={`rounded-xl border p-5 ${
        highlight ? highlightClasses[highlight] : 'bg-white border-slate-200'
      }`}
    >
