...
import { hotel, latestCycle } from '@/data/hotel-data'
import { cn } from '@/lib/utils'

const navItems = [
  { href: '/dashboard', label: 'Dashboard', Icon: LayoutDashboard, exact: true },
  { href: '/dashboard/recommendations', label: 'Recomendaciones', Icon: ListChecks, exact: false },
    ...
    <aside className="fixed left-0 top-0 h-screen w-64 bg-white border-r border-slate-200 flex flex-col z-10">
      ...
      <div className="px-4 py-4 border-t border-slate-100">
        <div className="bg-slate-50 rounded-xl p-3 mb-3">
          ...
          <p className="text-xs font-semibold text-slate-700">
            hace {latestCycle.days_ago} días · {latestCycle.findings_count} hallazgos
          </p>
        </div>
        <button
          ...
          onClick={() => alert('En el producto real, esto lanzaría un nuevo ciclo de análisis vía n8n.')}
        >
          <Zap size={15} />
          Nuevo análisis
        </button>
      ...
      <div className="px-5 py-3 border-t border-slate-100">
        <p className="text-xs text-slate-400">
          by{' '}
          <span className="font-semibold text-slate-500">EstudioTech</span>
        </p>
      </div>
    </aside>
  )
